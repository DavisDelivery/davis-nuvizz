// test/stop-lookup-promote.test.mjs — THE PROMOTED CALL: one NuVizz call, on request, and
...
// firestore data allow a promoted nuvizz call." The rules are pure (src/lib/stop-lookup.js)
// and the endpoint is the one door that spends (netlify/functions/stop-lookup-promote.mts).
...
delete process.env.STOP_LOOKUP_PROMOTE;
...
import {
  switchOn, promoteAvailability, promotedRecordDay, promotedStoreDecision, promotedRecord, promoteOutcome, promotedSource,
} from '../src/lib/stop-lookup.js';
import { scansEnabled } from '../netlify/functions/lib/nuvizz-scan.mts';

const T = 'davis';
const FN = 'https://x.netlify.app/.netlify/functions/stop-lookup-promote';
...
async function call(seed, qs, vendor) {
  ...
  try {
    const handler = (await import('../netlify/functions/stop-lookup-promote.mts')).default;
...
test('availability: mirror wins, then the promote switch, then the scans switch — each with its own sentence', () => {
  assert.equal(promoteAvailability({ mirror: true, promoteSwitch: 'on' }).reason, 'mirror');
  assert.equal(promoteAvailability({ promoteSwitch: 'off' }).reason, 'switch');
  assert.equal(promoteAvailability({ scansSwitch: 'false' }).reason, 'scans');
  const ok = promoteAvailability({});
...
test('THE SCANS SWITCH IS READ THE SAME WAY scansEnabled() READS IT — only the literal false turns it off', () => {
  ...
  try {
    for (const v of ['false', 'true', '0', 'off', '', 'FALSE', 'no']) {
      ...
      const ours = promoteAvailability({ scansSwitch: v }).available;
      assert.equal(ours, theirs, `NUVIZZ_SCANS_ENABLED=${JSON.stringify(v)}: promote ${ours} vs scansEnabled ${theirs}`);
    }
...
test('the record day: delivered, else arrived, else planned ETA, else the window — else null', () => {
  assert.equal(promotedRecordDay({ deliveredDTTM: '2026-09-10T13:44', arrivalDTTM: '2026-09-11T09:00' }), '2026-09-10');
  assert.equal(promotedRecordDay({ arrivalDTTM: '2026-09-11T09:00' }), '2026-09-11');
  assert.equal(promotedRecordDay({ plannedEtaDTTM: '2026-09-12T09:00' }), '2026-09-12');
  assert.equal(promotedRecordDay({ scheduledFrom: '2026-09-13T08:00:00' }), '2026-09-13');
  assert.equal(promotedRecordDay({ scheduledFrom: 'garbage' }), null);
  assert.equal(promotedRecordDay(null), null);
});

test('FILE PAST DAYS ONLY: today and the future are shown, not filed; no day is shown, not filed', () => {
  assert.equal(promotedStoreDecision({ day: '2026-09-10', today: '2026-09-19' }).store, true);
  assert.equal(promotedStoreDecision({ day: '2026-09-19', today: '2026-09-19' }).store, false);
  assert.equal(promotedStoreDecision({ day: '2026-09-25', today: '2026-09-19' }).reason, 'live');
  assert.equal(promotedStoreDecision({ day: null, today: '2026-09-19' }).reason, 'no-day');
  assert.equal(promotedStoreDecision({ day: '2026-09-10', today: null }).store, false, 'no clock → never file');
});

test('the filed record carries the day, the customer key and honest provenance', () => {
  const r = promotedRecord({ stopNbr: '007180002', customerMatchKey: 'stale' }, { day: '2026-09-10', at: '2026-09-19T03:00:00Z', by: 'tina', matchKey: 'fresh' });
  assert.equal(r.date, '2026-09-10');
  assert.equal(r.customerMatchKey, 'fresh');
  assert.equal(r.promoted, true);
  assert.equal(r.promoted_by, 'tina');
  assert.equal(r.promoted_from, 'stop-lookup');
  assert.equal(promotedRecord({ stopNbr: '1', customerMatchKey: 'kept' }, { day: 'x' }).customerMatchKey, 'kept');
});

test('the outcome says whether a call was SPENT: a breaker/kill-switch refusal is not a charge', () => {
  assert.deepEqual([promoteOutcome({ ok: true, stop: {} }).ok, promoteOutcome({ ok: true, stop: {} }).spent], [true, true]);
  assert.equal(promoteOutcome({ ok: false, reason: 'scans_disabled' }).spent, false);
  assert.equal(promoteOutcome({ ok: false, reason: 'NuVizz circuit breaker open — refusing /stop/info (DAVIS)' }).reason, 'breaker');
  assert.equal(promoteOutcome({ ok: false, reason: 'NuVizz circuit breaker open — refusing /stop/info (DAVIS)' }).spent, false);
  assert.equal(promoteOutcome({ ok: false, reason: 'http_404' }).reason, 'not_found');
  assert.equal(promoteOutcome({ ok: false, reason: 'http_404' }).spent, true);
  assert.equal(promoteOutcome({ ok: false, reason: 'not_found' }).reason, 'not_found');
  assert.equal(promoteOutcome({ ok: false, reason: 'http_500' }).reason, 'error');
  assert.equal(promoteOutcome(null).reason, 'error');
});

test('the ledger row for a promoted answer is a FOUND row that names its price', () => {
  const r = promotedSource({ day: '2026-09-10' });
  assert.equal(r.key, 'nuvizz');
  assert.equal(r.state, 'found');
  assert.match(r.note, /one call/);
  assert.match(promotedSource({}).note, /no delivery day/);
});
...
test('A PAST ORDER NUVIZZ HAS: exactly one call, FILED in the warehouse with a pointer, answered as found', async () => {
  ...
  assert.equal(body.promoted.attempted, true);
  assert.equal(body.promoted.ok, true);
  assert.equal(body.promoted.day, day);
  assert.equal(body.promoted.decision, 'past');
  assert.equal(body.promoted.stored.ok, true);
  assert.equal(body.promoted.stored.created, true);
  assert.equal(body.promoted.stored.pointer, true);
  ...
  assert.equal(rec.promoted_from, 'stop-lookup');
...
test('TODAY\'S ORDER: one call, shown, NOT filed — the board scan owns today', async () => {
  ...
  assert.equal(body.nuvizzCalls, 1);
  assert.equal(body.promoted.ok, true);
  assert.equal(body.promoted.decision, 'live');
  assert.equal(body.promoted.stored, null);
...
test('NEVER SPEND ON AN ORDER WE HOLD: a PRO-index hit answers "on file" with ZERO calls', async () => {
  ...
  assert.equal(body.promoted.attempted, false);
  assert.equal(body.promoted.reason, 'on-file');
  assert.deepEqual(body.promoted.days, ['2026-09-15']);
});
...
test('NUVIZZ HAS NOTHING EITHER: one call, an honest sentence, nothing filed', async () => {
  ...
  assert.equal(body.promoted.attempted, true);
  assert.equal(body.promoted.ok, false);
  assert.equal(body.promoted.reason, 'not_found');
  assert.match(body.promoted.text, /no order by this number/);
  assert.equal([...fake.store.keys()].some((k) => k.startsWith('history_days/')), false);
});

test('STOP_LOOKUP_PROMOTE=off: refused before the wire, zero calls, the switch named', async () => {
  process.env.STOP_LOOKUP_PROMOTE = 'off';
  try {
    const { fake, body } = await call({}, 'stop=7180002');
    assert.equal(body.nuvizzCalls, 0);
    assert.equal(fake.log.other.length, 0);
    assert.equal(body.promoted.attempted, false);
    assert.equal(body.promoted.reason, 'switch');
    assert.equal(body.promote.available, false);
  } finally { delete process.env.STOP_LOOKUP_PROMOTE; }
});
...
test('NUVIZZ_SCANS_ENABLED=false: the site-wide kill switch is honoured, zero calls', async () => {
  ...
  try {
    ...
    assert.equal(body.nuvizzCalls, 0);
    assert.equal(fake.log.other.length, 0);
    assert.equal(body.promoted.reason, 'scans');
...
test('A SEALED RECORD ALREADY THERE IS NEVER OVERWRITTEN — the create is atomic and loses', async () => {
  ...
  assert.equal(body.nuvizzCalls, 1);
  assert.equal(body.promoted.stored.created, false, 'the create lost to the sealed record');
