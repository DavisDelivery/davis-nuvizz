...
import { buildRouteCreateBody, buildCancelLoadBody, buildCancelStopBody, buildOpRequest } from '../netlify/functions/lib/nuvizz-write-ops.mts';
...
test('cancelLoad / cancelStop bodies validate against CancelLoad / CancelStop', () => {
  assert.deepEqual(validate(bodySchemaFor('/load/cancel/{companyCode}'), buildCancelLoadBody({ loadId: '6a438e9d52ef82bd1ed4516b', reasonComments: 'Route emptied in Dispatch Map (Cancel route)' })), []);
  assert.deepEqual(validate(bodySchemaFor('/load/cancel/{companyCode}'), buildCancelLoadBody({ loadNbr: 'DAVIS000203261' })), []);
...
test('the ops the app POSTs really do target the paths the document declares', () => {
  ...
  const cases = [
    ['cancelLoad', { loadId: '6a438e9d52ef82bd1ed4516b' }, '/load/cancel/{companyCode}'],
...
test('a route cancel has a DOCUMENTED endpoint, and it takes no load header for a Vehicle Type to refuse', () => {
  // Why §Y exists: load/edit (the remove-every-delivery emulation) is not even in this
  // document, while "Cancel Load" is — and its body cannot carry a vehicleType.
  const schema = deref(bodySchemaFor('/load/cancel/{companyCode}'));
  assert.match(SPEC.paths['/load/cancel/{companyCode}'].post.description, /Delete\/Reject a Load/);
  assert.deepEqual(schema.required, ['reasonCode']);
  assert.deepEqual(Object.keys(schema.properties).sort(), ['loadId', 'loadNbr', 'reasonCode', 'reasonComments']);
  assert.equal('vehicleType' in schema.properties, false);
  assert.equal('loadHeader' in schema.properties, false);
});
