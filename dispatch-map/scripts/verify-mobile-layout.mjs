...
const SCREENS = [
  ...
  // and the guard only ever saw whichever one was the default. A mode nobody sweeps is a mode
  // that ships its phone layout untested, which is the whole reason this file exists. Each is
  // a full pass: different sub-tab strip, different panel body, same 390 and 360 screens.
  { key: 'routing-drivers', label: 'Routing (beta) — Routes / Drivers rail', nav: /routing/i,
...
function stubRoutes(page, emailHtml) {
  return page.route('**/.netlify/functions/**', (route) => {
    ...
    if (u.includes('day-completion') && u.includes('history=1')) return R({ ok: true, days: [
      ...
      { date: '2026-08-18', open: 9, completionRate: 0.97, manualRate: 0.1, counts: {}, reconciled: { openAtSnapshot: 9, closedAfter: 8, failedAfter: 0, cancelledAfter: 0, stillOpen: 1, lateCloseRate: 0.89 } },
    ] });
...
// The measurement itself lives in scripts/lib/layout-measure.mjs — shared with the tablet
// guard so the two cannot drift about what counts as a collision.
