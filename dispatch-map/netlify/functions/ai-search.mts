...
const CHAT_SYSTEM = [
  ...
  'many stops record hours only as FREE TEXT inside dock_notes or appointment_notes',
  '(e.g. "receiving 8-3", "deliver before noon", "appt M-F 7am-2pm"). When the question is',
  'about hours, scan hours_summary AND dock_notes AND appointment_notes, and report a stop',
  'if hours appear in ANY of them.',
  ...
  '',
  'Be concise and practical — a dispatcher is reading this on a screen between calls.',
  'Do not invent stops or facts not present in the data. If nothing matches, say so.',
  '',
  'At the very END of your answer, output exactly one final line:',
  'MATCHED_PRO_IDS: <comma-separated pro values for every stop you referenced>',
  'Use the stop "pro" values verbatim (they are strings; preserve leading zeros).',
  'If no stops are referenced, output "MATCHED_PRO_IDS:" with nothing after it.',
].join('\n');
