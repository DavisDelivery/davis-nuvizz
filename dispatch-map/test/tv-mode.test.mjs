...
import { isTvPath, tvRailRows, tvVerdict, tvFeedStale, TV_RAIL_LIMIT } from '../src/lib/tv-mode.js';
...
test('"nothing needs a call" is SAID, because an empty rail and a broken one look the same', () => {
  assert.deepEqual(tvVerdict({ urgent: 0, amber: 0 }), { tone: 'clear', text: 'Nothing needs a call' });
});
...
test('A DEAD FEED BEATS A CLEAN BOARD — the wall must never print "all clear" over stale counts', () => {
  ...
  assert.equal(tvVerdict({ urgent: 0, amber: 0, stale: true }).tone, 'stale');
  assert.equal(tvVerdict({ urgent: 9, amber: 3, stale: true }).tone, 'stale',
