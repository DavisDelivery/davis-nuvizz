...
export interface EngineConfig {
  ...
  // ── Phase 2.12: PER-DRIVER skid caps ──────────────────────────────────────
  // The caps above are a FLEET statistic — box hard 22 is the p95 of all box
  // trips, while the same study puts the MEDIAN box trip at 14. Loading every
  // box driver to p95 is loading all of them like the heaviest one; most box
  // drivers run 12-15 skids and only a few take 17-18. The solver now bounds
  // each driver by the most THEY have actually carried in one trip, clamped at
  // the class cap (so a data quirk can't invent a 38-skid box truck) and
  // floored here (so a driver whose observed days were all small doesn't get a
  // 4-skid truck). Because of that clamp a per-driver cap is always <= the class
...
export function engineConfigDefaults(env: Record<string, string | undefined> = process.env): EngineConfig {
  ...
  return {
    ...
    // 10 = the study's box p50 (14) with room under it. A driver has to clear
    // min_observation_days (10) before their OWN envelope is used at all, so
    // this floor only catches a qualified driver whose observed trips were
